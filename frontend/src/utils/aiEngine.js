// ============================================================
// AI Engine - Crime Analysis Algorithms
...
// Crime type encoding for vectorization
...
export function findSimilarCrimes(targetFir, allFirs, topN = 5) {
    ...
    const results = allFirs
        ...
        .map(fir => {
            ...
            // Factor analysis for explainability
            const factors = [];
...
export function recommendSuspects(targetFir, criminals, allFirs) {
    return criminals
        .map(criminal => {
            let score = 0;
            const reasons = [];

            // Crime type match (35% weight)
...
export function identifyHotspots(firs) {
    const locationMap = {};
...
export function calculatePriority(fir, evidenceList, cases) {
    const weights = {
        severity: 0.30,
...
export function analyzeNetwork(criminals) {
    const nodes = criminals.map(c => ({
        id: c.criminalId,
        name: c.name,
        riskLevel: c.riskLevel,
        status: c.status,
        crimes: c.crimeHistory || [],
        connections: 0,
    }));
    ...
    criminals.forEach(c => {
        (c.associates || []).forEach(assocId => {
            ...
            if (!edgeSet.has(key)) {
                ...
                // Find shared crimes / cases
                const sharedCrimes = assoc
                    ? c.crimeHistory?.filter(ct => assoc.crimeHistory?.includes(ct)) || []
                    : [];
                const sharedCases = assoc
                    ? (c.relatedCases || []).filter(cs => (assoc.relatedCases || []).includes(cs))
                    : [];
                ...
                edges.push({
                    ...
                    strength: sharedCrimes.length + sharedCases.length,
                });
            }
        });
    });
    ...
    // Identify key players (most connected)
